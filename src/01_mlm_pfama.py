...
def read_fasta(
        path: str = "datasets/homeodomain_seed.fasta"
    ) -> list[SeqRecord]:
    with open(path) as p:
...
def main():
    ...
    model = ProteinLanguageModel(vocab_size=vocab_size, 
                                 embedding_dim=128, 
                                 num_heads=4, 
                                 num_layers=4
                                 )
    ...
    for n in range(num_epochs):
        ...
        mask_indices = np.array([
            np.random.choice(X_train.shape[1], size=int(0.15 * X_train.shape[1]), replace=False)
            for _ in range(X_train.shape[0])
        ]) 

        row_indices = torch.arange(X_train.shape[0]).unsqueeze(1).expand_as(torch.tensor(mask_indices))
